...
namespace HoloToolkitExtensions.RemoteAssets
{
    ...
    {
        public static Texture2D LoadDDSManual(Stream stream, string name)
        {
            try
            {
                byte[] ddsBytes = stream.ReadFully();
        ...
        public static Texture2D LoadTexture(Uri textureUri, bool normalMap = false)
        {
            Debug.Log("Loading texture from Uri " + textureUri);

            string ext = Path.GetExtension(textureUri.ToString()).ToLower();
            string baseName = textureUri.GetFileNameWithoutExtension();

            using (var stream = textureUri.GetStream())
            {
                if (ext == ".png" || ext == ".jpg")
                {
                    Texture2D t2d = new Texture2D(1, 1);
                    t2d.LoadImage(stream.ReadFully());
                    if (normalMap)
                ...
                {
                    Texture2D returnTex = LoadDDSManual(stream, baseName);
                    if (normalMap)
                ...
                {
                    Debug.Log("texture not supported : " + textureUri);
                }
            }
            return null;
        }
