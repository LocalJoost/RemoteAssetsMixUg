...
namespace HoloToolkitExtensions.RemoteAssets
{
    public class RemoteOBJLoader
    {
        ...
        private static Color ParseColorFromCMPS(string[] cmps, float scalar = 1.0f)
        {
        ...
        private static Material[] LoadMTLFile(Uri uri)
        {
            Debug.Log("Loading material from Uri " + uri);
            Material currentMaterial = null;
            List<Material> matlList = new List<Material>();
            using (var reader = uri.GetReader())
            {
                string ln;
                while ((ln = reader.ReadLine()) != null)
                {
                    ...
                    if (cmps[0] == "newmtl")
                    {
                        ...
                        currentMaterial = new Material(Shader.Find("HoloToolkit/StandardFast"));
                    ...
                    {
                        //TEXTURE
                        var texturePath = new Uri(uri.GetBaseUri(), data);
                        currentMaterial.SetTexture("_MainTex", RemoteTextureLoader.LoadTexture(texturePath));
                    }
                    else if (cmps[0] == "map_Bump")
                    {
                        ...
                        {
                            var texturePath = new Uri(uri.GetBaseUri(), data);
                            currentMaterial.SetTexture("_BumpMap", RemoteTextureLoader.LoadTexture(texturePath, true));
        ...
        public static GameObject LoadOBJFromUri(string name, string objUrl)
        {
#if UNITY_EDITOR
...
#endif
            return LoadOBJFromUri(new Uri(objUrl), name);
        }

        private static GameObject LoadOBJFromUri(Uri uri, string name)
        {

            Debug.Log("Loading objects from Uri " + uri);
                ...
                while ((ln = reader.ReadLine()) != null)
                {
                    if (ln.Length > 0 && ln[0] != '#')
                    {
                        ...
                        if (cmps[0] == "mtllib")
                        {
                            ...
                            var mtlUri = new Uri(baseUri, cmps[1]);
                            materialCache = LoadMTLFile(mtlUri);
